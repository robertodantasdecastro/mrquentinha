...
57. [x] Ops/Web-02/03/2026 Rede hibrida: admin/portal/webclient passaram a consumir API via proxy same-origin (Nginx -> backend local) e `Portal CMS` ganhou chave de API publica mobile (AWS DNS/IP) sem uso de dominio `mrquentinha.com.br` - concluida em 02/03/2026.
58. [x] Ops/Web-02/03/2026 Validacao final de conectividade: smoke de `admin/www/app` + API publica AWS validado; `start_vm_prod.sh` ajustado para perfil t3.micro com fail-fast/log tail por servico - concluida em 02/03/2026.
